            ...
            @if($label != __('dashboard.disposition_letter') && !(auth()->user()->role == 'staff' && $label == __('dashboard.active_user')))
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="cardOpt3">
                        ...
                        @elseif($label == __('dashboard.perdir'))
                            <a class="dropdown-item"
                               href="{{ route('transaction.perdir.index') }}">{{ __('dashboard.view_more') }}</a>

                        @elseif($label == __('dashboard.kpts'))
                            <a class="dropdown-item"
                               href="{{ route('transaction.kpts.index') }}">{{ __('dashboard.view_more') }}</a>

                        @elseif($label == __('dashboard.pemberitahuan'))
                            <a class="dropdown-item"
                               href="{{ route('transaction.pemberitahuan.index') }}">{{ __('dashboard.view_more') }}</a>

                        @elseif($label == __('dashboard.pakta'))
                            <a class="dropdown-item"
                               href="{{ route('transaction.pakta.index') }}">{{ __('dashboard.view_more') }}</a>

                        @elseif($label == __('dashboard.notulen'))
                            <a class="dropdown-item"
                               href="{{ route('transaction.notulen.index') }}">{{ __('dashboard.view_more') }}</a>
