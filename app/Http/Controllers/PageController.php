...
class PageController extends Controller
{
    ...
    {
        // Menghitung total surat dari semua jenis
        $totalOutgoingLetter = Letter::outgoing()->count();
        $totalPerdirLetter = Letter::perdir()->count();
        $totalKptsLetter = Letter::kpts()->count();
        $totalPemberitahuanLetter = Letter::pemberitahuan()->count();
        $totalPaktaLetter = Letter::pakta()->count();
        $totalNotulenLetter = Letter::notulen()->count();

        $totalLetterTransaction = $totalOutgoingLetter + $totalPerdirLetter + $totalKptsLetter + $totalPemberitahuanLetter + $totalPaktaLetter + $totalNotulenLetter;
        ...
        return view('pages.dashboard', [
            ...
            'totalPerdirLetter' => $totalPerdirLetter,
            'totalKptsLetter' => $totalKptsLetter,
            'totalPemberitahuanLetter' => $totalPemberitahuanLetter,
            'totalPaktaLetter' => $totalPaktaLetter,
            'totalNotulenLetter' => $totalNotulenLetter,
            'todayLetterTransaction' => $totalLetterTransaction,
            'activeUser' => User::active()->count(),
            'percentageLetterTransaction' => 0, // Dummy value for now
            'percentageOutgoingLetter' => 0,  // You can replace with actual calculations
            'percentagePerdirLetter' => 0,
            'percentageKptsLetter' => 0,
            'percentagePemberitahuanLetter' => 0,
            'percentagePaktaLetter' => 0,
            'percentageNotulenLetter' => 0,
        ]);
    }
